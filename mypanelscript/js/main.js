...
function init() {
  ...
  //     folderCount ++;
  //     foHTML += '<div class="folder-group" data-folderName="'+fobName+'">';
  ...
  //     foHTML +=   '<ul  style="width:100%">';
  //     if(fob.files && fob.files.length > 0){
  //       _.each(fob.files, function(fobFile){
  //         foHTML +=  '<li class="file-name" data-fileName="'+fobFile+'"> <button  class="button" style="width:100%; margin-bottom:1px"> &nbsp;&nbsp;' + spacer + fileSpacer + fobFile + '</button></li>';
  //         // foHTML +=  ' ';
  //         // foHTML +=  '&nbsp;&nbsp;' + spacer + fileSpacer + fobFile + '';
  //         foHTML += '</li>';
  //       });
  //     }
  //     foHTML +=  ' </ul>';
  //     if(fob.folders && _.size(fob.folders) > 0){
  //       // foHTML +=   '<ul class="button" >';
...
function storeLocalSettings () {
    ...
    localStorage.setItem (
      "com.mustthank.panelScript.toggled",
      JSON.stringify(toggled)
    );
}
function storeFolderPath(folderPath){
  localStorage.setItem (
    "com.mustthank.panelScript.folderPath",
    folderPath
  );
}
function getLocalSettings () {
  csInterface.evalScript("$.setAppName("+$.stringify(appName)+")");
  var toggled = localStorage.getItem("com.mustthank.panelScript.toggled");
  if (toggled) {
  ...
  var folderPath = localStorage.getItem("com.mustthank.panelScript.folderPath");
  if(folderPath){
